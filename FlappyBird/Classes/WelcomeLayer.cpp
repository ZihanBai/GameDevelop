...
bool WelcomeLayer::init(){
    ...
    this->bird->setTag(BIRD_PRITE_TAG);
    this->bird->setPosition(Point(origin.x + visibleSize.width / 2,origin.y + visibleSize.height /2 ));
    this->bird->idle();
...
void WelcomeLayer::scrollLand(float dt){
    this->land1->setPositionX(this->land1->getContentSize().width - 2.0f);
    this->land2->setPositionX(this->land1->getContentSize().width + this->land1->getContentSize().width - 2.0f);
...
void WelcomeLayer::menuStartCallBack(Object *sender){
    ...
    this->removeChildByTag(BIRD_PRITE_TAG);
